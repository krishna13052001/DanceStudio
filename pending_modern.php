...
<table border="1">
			...
			<tbody>
				<?php
					include('connection.php');
					$query=mysqli_query($conn,"select * from `tbl_chor_reg` where dance_type='Modern Dance'");
					while($row=mysqli_fetch_array($query)){
						?>
						<tr>
						   
<!--							<form action="pending_modern.php" method="POST">-->
								...
								<?php 
                                    $q=mysqli_query($conn,"select * from request where dance_type='Modern Dance' and student_name='$name' and status='pending'");
